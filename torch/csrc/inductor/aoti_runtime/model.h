...
using CPUPtr = std::unique_ptr<void, std::function<void(void*)>>;

CPUPtr RAII_cpuMalloc(size_t num_bytes) {
  void* data_ptr = std::malloc(num_bytes);
  if (!data_ptr) {
    throw std::bad_alloc();
  }
  auto deleter = [](void* ptr) { std::free(ptr); };
  return CPUPtr(data_ptr, deleter);
}
...
#if defined(USE_CUDA) || defined(USE_XPU)
  // Holds the blob storage for constants' at::Tensor for GPU.
